...
namespace TeamGipsy.Model.SqliteControl
{
    public class Select
    {
        public Select()
        ...
        public void LoadGlobalConfig()
        {
            ...
            if (TABLE_NAME == "StdJp_Mid" || TABLE_NAME == "Goin"
                ...
                || TABLE_NAME == "SAT_2" || TABLE_NAME == "Level4_1"
                || TABLE_NAME == "Level4luan_2" || TABLE_NAME == "Level8_1"
                || TABLE_NAME == "Level8luan_2")
            {
                TABLE_NAME = "CET4_1";
                Update.CommandText = "UPDATE Global SET currentBookName = 'CET4_1'";
                Update.ExecuteNonQuery();
            }
        }
        ...
        public void CleanupJapaneseData()
        {
            SQLiteCommand Update = DataBase.CreateCommand();
            Update.CommandText = "DELETE FROM Count WHERE bookName IN ('Goin','StdJp_Mid')";
            try { Update.ExecuteNonQuery(); } catch { }

            Update.CommandText = "UPDATE Global SET currentBookName = 'CET4_1' WHERE currentBookName IN ('Goin','StdJp_Mid')";
            try { Update.ExecuteNonQuery(); } catch { }

            Update.CommandText = "DROP TABLE IF EXISTS Goin";
            try { Update.ExecuteNonQuery(); } catch { }

            Update.CommandText = "DROP TABLE IF EXISTS StdJp_Mid";
            try { Update.ExecuteNonQuery(); } catch { }
        }

        public void CleanupRemovedEnglishBooks()
        {
            SQLiteCommand Update = DataBase.CreateCommand();
            Update.CommandText = "DELETE FROM Count WHERE bookName IN ('GMAT_3','GRE_2','SAT_2','Level4_1','Level4luan_2','Level8_1','Level8luan_2')";
            try { Update.ExecuteNonQuery(); } catch { }

            Update.CommandText = "UPDATE Global SET currentBookName = 'CET4_1' WHERE currentBookName IN ('GMAT_3','GRE_2','SAT_2','Level4_1','Level4luan_2','Level8_1','Level8luan_2')";
            try { Update.ExecuteNonQuery(); } catch { }

            string[] removed = new string[] { "GMAT_3", "GRE_2", "SAT_2", "Level4_1", "Level4luan_2", "Level8_1", "Level8luan_2" };
            foreach (var t in removed)
            {
                Update.CommandText = $"DROP TABLE IF EXISTS {t}";
                try { Update.ExecuteNonQuery(); } catch { }
            }
        }
